/**
 * Centralized logging service for the admin panel
...
class LoggingService {
  ...
  entityOperation(operation: string, entityId?: string, data?: any): void {
    this.info(`Entity Operation: ${operation}${entityId ? ` (${entityId})` : ''}`, 'ENTITY', data);
  }

  // Admin management logging
  adminOperation(operation: string, username?: string, data?: any): void {
    this.info(`Admin Operation: ${operation}${username ? ` (${username})` : ''}`, 'ADMIN', data);
  }

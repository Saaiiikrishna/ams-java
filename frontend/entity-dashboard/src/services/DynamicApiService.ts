/**
 * Dynamic API Service for Entity Dashboard
...
class DynamicApiService {
  ...
  public async getApiBaseUrl(): Promise<string> {
    ...
    if (window.location.port === '8080') {
      console.log('✅ Entity Dashboard: Using same-origin API calls (relative URLs)');
      return '';
    }

    // If we have a cached working server, use it
    if (this.currentServerUrl && this.isServerCacheValid(this.currentServerUrl)) {
      console.log('✅ Entity Dashboard: Using cached server:', this.currentServerUrl);
    ...
    console.warn('⚠️ Entity Dashboard: Using fallback server:', fallbackUrl);
    return fallbackUrl;
  }
  ...
  private async discoverServer(): Promise<string | null> {
    ...
    console.log('🚀 Entity Dashboard: Starting server discovery...');

    try {
      // Get potential server URLs
      const serverUrls = this.getPotentialServers();
      console.log('🔍 Entity Dashboard: Testing', serverUrls.length, 'potential servers...');
      ...
      for (let i = 0; i < results.length; i++) {
        ...
        if (result.status === 'fulfilled' && result.value) {
          ...
          console.log('✅ Entity Dashboard: Found working server:', workingUrl);
      ...
      console.warn('❌ Entity Dashboard: No working servers found');
      ...
      console.error('❌ Entity Dashboard: Server discovery failed:', error);
      this.discoveryCallbacks.forEach(callback => callback(null));
      this.discoveryCallbacks = [];
      return null;
    } finally {
  ...
  private async testServerWithTimeout(url: string): Promise<boolean> {
    ...
    try {
      ...
      console.log(`🔍 Entity Dashboard: ${url} - ${isWorking ? '✅ PASS' : '❌ FAIL'} (${responseTime}ms)`);
      return isWorking;

    } catch (error) {
      console.log(`🔍 Entity Dashboard: ${url} - ❌ FAIL (${error instanceof Error ? error.message : 'Unknown error'})`);
  ...
  private loadCachedServer(): void {
    try {
      ...
      if (cached) {
        ...
        console.log('📱 Entity Dashboard: Loaded cached server:', cached);
      }
    } catch (error) {
      console.warn('⚠️ Entity Dashboard: Failed to load cached server:', error);
    }
  }
  ...
  private cacheServer(url: string): void {
    try {
      ...
      console.log('💾 Entity Dashboard: Cached server:', url);
    } catch (error) {
      console.warn('⚠️ Entity Dashboard: Failed to cache server:', error);
    }
  }
  ...
  private startPeriodicHealthCheck(): void {
    setInterval(async () => {
      if (this.currentServerUrl && !this.isDiscovering) {
        ...
        if (!isHealthy) {
          console.warn('⚠️ Entity Dashboard: Current server is unhealthy, rediscovering...');
  ...
  public async forceRediscovery(): Promise<string | null> {
    console.log('🔄 Entity Dashboard: Forcing server rediscovery...');
    this.currentServerUrl = null;

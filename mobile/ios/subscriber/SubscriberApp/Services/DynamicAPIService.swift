...
/**
 * Dynamic API Service with mDNS discovery and automatic server detection
...
class DynamicAPIService: ObservableObject {
    ...
    private let healthEndpoint = "subscriber/health"
    ...
    func discoverServer() {
        ...
        if let cachedServer = getCachedServer() {
            print("🔍 Testing cached server: \(cachedServer)")
            testServer(url: cachedServer) { [weak self] success in
                if success {
                    print("✅ Cached server is working: \(cachedServer)")
    ...
    private func performServerDiscovery() {
        ...
        discoverViaMDNS { [weak self] discoveredURL in
            if let url = discoveredURL {
                print("✅ mDNS discovery successful: \(url)")
                self?.setCurrentServer(url)
            } else {
                print("❌ mDNS discovery failed, trying fallback servers...")
    ...
    private func discoverViaMDNS(completion: @escaping (String?) -> Void) {
        ...
        if status == 0, let result = result {
            ...
            print("🏠 Resolved \(hostname) to \(ipString)")
    ...
    private func tryFallbackServers() {
        ...
        group.notify(queue: .main) { [weak self] in
            if let server = workingServer {
                print("✅ Fallback server found: \(server)")
    ...
    private func setCurrentServer(_ url: String) {
        ...
        print("🎯 Current server set to: \(url)")
    }
    
    private func getCachedServer() -> String? {
        return UserDefaults.standard.string(forKey: "cached_server_url")
    }
    
    private func cacheServer(_ url: String) {
        UserDefaults.standard.set(url, forKey: "cached_server_url")
    }
    
    func clearCache() {
        UserDefaults.standard.removeObject(forKey: "cached_server_url")
    ...
    func getSubscriberBaseURL() -> String {
        let baseURL = getBaseURL()
        return baseURL.hasSuffix("/") ? baseURL + "subscriber/" : baseURL + "/subscriber/"
    }
}

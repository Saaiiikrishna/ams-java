...
/**
 * Enhanced mDNS discovery using JmDNS library for better compatibility
 * This provides a fallback when Android NSD doesn't work properly
 */
class EnhancedMDnsDiscovery(private val context: Context) {
    
    companion object {
        private const val TAG = "EnhancedMDnsDiscovery"
    ...
    data class DiscoveredService(
        ...
        val subscriberHealthUrl: String get() = "${baseUrl}subscriber/health"
    ...
    fun discoverServices(): Flow<List<DiscoveredService>> = flow {
        Log.i(TAG, "🚀 Starting enhanced mDNS service discovery...")
        ...
        try {
            withTimeout(DISCOVERY_TIMEOUT_MS) {
                ...
                repeat(8) { // Check 8 times over 8 seconds
                    ...
                    if (currentServices.size != lastEmittedSize) {
                        ...
                        Log.i(TAG, "📡 Emitted ${currentServices.size} discovered services")
                    }
                }
            ...
            Log.i(TAG, "⏰ Discovery timeout reached, found ${discoveredServices.size} services")
            emit(discoveredServices.values.toList())
        } catch (e: Exception) {
            Log.e(TAG, "❌ Enhanced discovery failed: ${e.message}", e)
            emit(emptyList())
        } finally {
            cleanup()
        }
        ...
        return withContext(Dispatchers.IO) {
            try {
                withTimeout(timeoutMs) {
                    ...
                    Log.i(TAG, "🎯 Sync discovery completed: ${services.size} services found")
                    services
                }
            } catch (e: Exception) {
                Log.e(TAG, "❌ Sync enhanced discovery failed: ${e.message}", e)
                emptyList()
            } finally {
                cleanup()
            }
    ...
    private suspend fun startJmDNSDiscovery() = withContext(Dispatchers.IO) {
        try {
            ...
            Log.i(TAG, "📱 Using WiFi IP: ${inetAddress.hostAddress}")
            
            // Create JmDNS instance
            jmdns = JmDNS.create(inetAddress, "AndroidClient")
            ...
            val serviceListener = object : ServiceListener {
                ...
                override fun serviceRemoved(event: ServiceEvent) {
                    Log.d(TAG, "🗑️ Service removed: ${event.name}")
                    discoveredServices.remove(event.name)
                }
                
                override fun serviceResolved(event: ServiceEvent) {
                    val info = event.info
                    Log.i(TAG, "✅ Service resolved: ${info.name}")
            ...
            Log.i(TAG, "👂 Started listening for services of type: $SERVICE_TYPE")
            
        } catch (e: Exception) {
            Log.e(TAG, "❌ Failed to start JmDNS discovery: ${e.message}", e)
            throw e
        }
    }
    ...
    private fun acquireMulticastLock() {
        try {
            if (multicastLock == null) {
                multicastLock = wifiManager.createMulticastLock("EnhancedMDnsDiscovery")

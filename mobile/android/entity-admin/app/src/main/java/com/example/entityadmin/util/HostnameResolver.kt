...
object HostnameResolver {
    ...
    private const val TAG = "EntityHostnameResolver"
    ...
    suspend fun resolveRestaurantLocal(context: Context): String? = withContext(Dispatchers.IO) {
        try {
            ...
            val knownIps = listOf("192.168.31.4", "192.168.31.209", "172.20.10.2")
            for (ip in knownIps) {
    ...
    private suspend fun scanForServer(deviceIp: String): String? = withContext(Dispatchers.IO) {
        try {
            ...
            val priorityIps = listOf(
                "$networkBase.4",   // Current known server IP (from mDNS discovery)
                "$networkBase.209", // Alternative server IP

...
object ServerDiscovery {
    ...
    private const val TAG = "EntityServerDiscovery"
    private const val PREFS_NAME = "entity_server_discovery"
    private const val KEY_DISCOVERED_SERVER = "discovered_server"
    private const val TEST_ENDPOINT = "api/auth/login"  // Test connectivity using login endpoint
    ...
    fun discoverServerUrl(context: Context): String {
        Log.i(TAG, "🚀 Starting comprehensive entity admin server discovery...")
        ...
        Log.i(TAG, "🌐 Starting fresh network discovery for entity admin backend server...")
        ...
        val fallbackServers = listOf(
            "http://192.168.31.4:8080/", // Current known server IP (from mDNS discovery)
    ...
    suspend fun discoverServerUrlAsync(context: Context): String = withContext(Dispatchers.IO) {
        Log.i(TAG, "🚀 Starting fast async entity admin server discovery...")
    ...
    fun testServerQuick(baseUrl: String): Boolean {
        return try {
            ...
            val request = Request.Builder()
                .url(url)
                .get()  // Use GET to test connectivity (will return 405 Method Not Allowed, but server is responding)
                .addHeader("User-Agent", "EntityAdminApp/1.0")
                .build()

            val response = quickClient.newCall(request).execute()
            val responseBody = response.body?.string()
            // Server is responding if we get any HTTP response (even 405, 400, etc.)
            val isResponding = response.code in 200..599

            Log.d(TAG, "📊 Response for $baseUrl:")
            Log.d(TAG, "   Status: ${response.code}")
            Log.d(TAG, "   Success: ${response.isSuccessful}")
            Log.d(TAG, "   Body: ${responseBody?.take(100)}")
            Log.d(TAG, "   Result: ${if (isResponding) "✅ PASS" else "❌ FAIL"}")
    ...
    suspend fun testServerHealth(baseUrl: String): Boolean = withContext(Dispatchers.IO) {
        try {
            ...
            val request = Request.Builder()
                .url(url)
                .get()  // Use GET to test connectivity
                .build()
            ...
            Log.d(TAG, "Connectivity test for $baseUrl: ${if (isResponding) "PASS" else "FAIL"}")
            response.close()
            return@withContext isResponding
        } catch (e: Exception) {

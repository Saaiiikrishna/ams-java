...
/**
 * Enhanced Service Discovery for Entity Admin with intelligent load balancing,
 * health monitoring, and circuit breaker patterns
 */
class EnhancedServiceDiscovery(private val context: Context) {
    
    companion object {
        private const val TAG = "EntityEnhancedServiceDiscovery"
    ...
    private suspend fun discoverServices() {
        Log.d(TAG, "🔍 Entity Admin: Starting enhanced service discovery...")
        ...
        try {
            ...
            val newInstances = discoveredServices.mapIndexed { index, service ->
                ...
                ServiceInstance(
                    ...
                    metadata = mapOf(
                        ...
                        "discovered_at" to System.currentTimeMillis().toString(),
                        "discovery_method" to "mdns",
                        "client_type" to "entity_admin"
                    )
                )
            }
            ...
            Log.i(TAG, "✅ Entity Admin: Discovered ${newInstances.size} service instances")
            _serviceUpdates.emit(getHealthyInstances())
            
        } catch (e: Exception) {
            Log.e(TAG, "❌ Entity Admin: Service discovery failed: ${e.message}")
        }
    }
    ...
    private fun updateServiceRegistry(newInstances: List<ServiceInstance>) {
        ...
        newInstances.forEach { instance ->
            ...
            if (existing != null) {
                ...
                Log.i(TAG, "➕ Entity Admin: Added new service instance: ${instance.id}")
            }
        }
    }
        ...
        scope.launch {
            while (isActive) {
                try {
                    ...
                    Log.e(TAG, "Entity Admin: Health monitoring error: ${e.message}")
    ...
    private suspend fun checkAllServicesHealth() {
        ...
        Log.d(TAG, "🏥 Entity Admin: Checking health of ${instances.size} service instances...")
    ...
    private suspend fun checkServiceHealth(instance: ServiceInstance) {
        try {
            ...
            val healthUrl = "${instance.baseUrl}api/health"
    ...
    private fun onHealthCheckSuccess(instance: ServiceInstance) {
        ...
        when (instance.circuitState) {
            CircuitState.HALF_OPEN -> {
                ...
                Log.i(TAG, "🔄 Entity Admin: Circuit breaker CLOSED for ${instance.id}")
            }
            CircuitState.OPEN -> {
        ...
        if (wasUnhealthy) {
            Log.i(TAG, "✅ Entity Admin: Service ${instance.id} is back online")
        }
    }
        ...
        if (instance.failureCount >= CIRCUIT_BREAKER_FAILURE_THRESHOLD) {
            when (instance.circuitState) {
                CircuitState.CLOSED -> {
                    ...
                    Log.w(TAG, "🚨 Entity Admin: Circuit breaker OPEN for ${instance.id} after ${instance.failureCount} failures")
                }
                CircuitState.HALF_OPEN -> {
                    instance.circuitState = CircuitState.OPEN
                    Log.w(TAG, "🚨 Entity Admin: Circuit breaker back to OPEN for ${instance.id}")
                }
                CircuitState.OPEN -> {
                    ...
                    if (timeSinceLastFailure > CIRCUIT_BREAKER_RECOVERY_TIMEOUT) {
                        ...
                        Log.i(TAG, "🔄 Entity Admin: Circuit breaker HALF_OPEN for ${instance.id}")
                    }
                }
            }
        }

        Log.w(TAG, "❌ Entity Admin: Health check failed for ${instance.id}: $error")
    }
    ...
    fun getBestServiceInstance(): ServiceInstance? {
        ...
        if (healthyInstances.isEmpty()) {
            Log.w(TAG, "⚠️ Entity Admin: No healthy service instances available")
            return null
        }
        ...
        for (instance in healthyInstances) {
            ...
            if (randomValue <= currentWeight) {
                Log.d(TAG, "🎯 Entity Admin: Selected service instance: ${instance.id} (weight: ${instance.weight})")
                return instance
            }
        }
    ...
    suspend fun forceRefresh() {
        Log.i(TAG, "🔄 Entity Admin: Forcing service discovery refresh...")
        discoverServices()
    }
